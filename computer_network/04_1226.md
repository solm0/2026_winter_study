...
- cs구조에서는 클라이언트 숫자가 많아질수록 소요시간이 많아진다.
  - N명이 F의 복사본을 원할 때, u는 업로드rate d는 다운로드rate D >= max{NF/u, F/d}
...
# socket with UDP
- 클라이언트와 서버 사이 connection이 없다. handshaking이 없다.
...
서버가 열린 socket한개로 여러 클라이언트 socket에서 받아서 서버의 출발포트를 명시안해도 됨?
...
장식 없음, 최소 기능 제공, 시도는 하지만 성공을 보장하지 않음.
