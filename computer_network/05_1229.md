...
# 여러개를 보내기
pipelined protocol
응답이 없더라도 여러개를 reuest 동시에보냄. request마다 타임아웃을 설정.

두 방법이 있음.

## Go-back-N
안 온 번호로 가서 걔부터 다시 보냄. 그 다음꺼 이미 보냈더라도 무시(in-order) 순서 틀린애들은 다 못갔다고 침. 그럼 한번에 얼마나 많이 보내야 할까? 네트워크 환경에 따라 바꿔야됨. window라는ㄱㅔ있음. 내가 보낼수있는 사이즈. adaptive함. 환경이 좋으면 많이 보냄. 안좋으면 window size를 줄임. max-low threshold 사이에서 늘렫다 줄였다 -> congestion control

10개를 보낼수있어서 보냈는데 응답이 오면 옆으로 옮겨감 -> sliding window

cumulative ack. 누적됨. 윈도우 sliding하면서.

window size 안에서 문제 n으로 돌아간 다음에 이미 보냈던 sequence까지 다시 보내준다.

## Selective Repeat
Go-back-N은 순서가 중요함. 7번못받았는데 8번오면 8번 버려야됨. -> 비효율적? 잘보낸건 보과낳면 안될까? 8,9,10이 오면 저장한다. 7이 나중에 오면 assemble해서 올려보내기. 선택해서 재전송. 문제있는 sequencenumber만 보냄.
...
100 ack가 제대로안갔는데 120이 갔으면 sender 입장에서는 앞에거가 잘 받았는지 판단하기 어렵다(실제로는 받았는데) -> 120을 주는 순간 앞에있는게 유실이 됐어도 잘 받았다고 확인이 된다면 재전송 안함. 근데 잘 받았는지 어떻게 아는데?
...
fast retransmit: duplicate ack를 3번 받으면 ack 안받은 segment를 재전송함. RTT가 길었는데 같은 ack를 3번: 안좋다는 신호임. 그러면 타임아웃까지 기다리지 말고 빨리 재전송을 해주자. 타임아웃을 기다리는게 더 안좋을수있다.문제가 생긴 패킷을 빠르게 조치해주기.

fast recovery랑 다름.그건 혼잡제어.
...
buffer에 얼마나있는지 보내주고
sender가 그것에따라 보내는양을 조절함. overflow오면 loss되니까.

그걸 헤더에다 너놓았다. receive window.
...
2000
threshold가 두개?
slowstart에서말하는 threshold는 문제없이 빠르게 잘 보낸 구간(그게 정확히 뭐지?). 히스토리를 보고. 증가시켜도 괜찮은 구간에서 exponential하게(slow start) 하다가 threshold넘으면 linear(AIMD) -> 문제 생기면 처음 값으로 쭉 내려감(AIMD) + threshold를 한 단계 낮춘다.

## Reno
2003
급격히 내려간 다음 시작점을 맨 첫번째 말고 threshold부터 AIMD(linear)로 다시 시작함. -> 회복이 빠름. fast recovery.

Tahoe는 급격하게 떨어지는 기준이 timeout임. 매우 안좋은 환경. triple duplicate ack도 안좋긴하지만 전조현상임. 그래서 그걸 기준으로 함.

## new Reno
duplicate ack났을때 바로 낮출거냐? reno는 문제생길때마다 threshold내려감 -> 환경이 좋아져도 시간걸림. new reno는 문제가 계속생기면 무시하고 threshold 유지하기.
